...
def compare_models(prompt: str) -> dict:
    ...
    """
    openai_result = call_openai(prompt, model=OPENAI_MODEL)
    openai_mini_result = call_openai(prompt, model=OPENAI_MINI_MODEL)
    gemini_result = call_gemini(prompt, model=GEMINI_MODEL)

    def _compute_cost(tokens: dict, pricing_key: str) -> float:
    ...
    gpt4o_response, gpt4o_latency, gpt4o_usage = openai_result
    gpt4o_mini_response, gpt4o_mini_latency, gpt4o_mini_usage = openai_mini_result
    gemini_response, gemini_latency, gemini_usage = gemini_result

    return {
        "gpt4o": {
            "response": gpt4o_response,
            "latency": gpt4o_latency,
            "cost": _compute_cost(gpt4o_usage, OPENAI_MODEL),
        ...
        "gpt4o_mini": {
            ...
            "cost": _compute_cost(gpt4o_mini_usage, OPENAI_MINI_MODEL),
        ...
        "gemini_flash": {
            ...
            "cost": _compute_cost(gemini_usage, GEMINI_MODEL),
...
def streaming_chatbot() -> None:
    ...
    print("Gemini 2.5 chatbot initialized. Type 'quit' or 'exit' to stop.")
    while True:
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = ["Prompt", "Model", "Response", "Latency", "Tokens", "Cost (USD)"]
    lines = [
        "| " + " | ".join(header) + " |",
        "| " + " | ".join(["---"] * len(header)) + " |",
    ]
